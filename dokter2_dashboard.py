...
class dokter2Dashboard:
    ...
    def build_ui(self):
        ...
        # FIX: Hapus padx/pady agar menempel penuh ke sisi window
        ...
        for top_icon in ["Y", "🖨️", "⬇️", "⤢"]:
             ctk.CTkButton(right_action, text=top_icon, width=35, height=35, fg_color="transparent", 
                          text_color="#555", hover_color="#eee", font=("Arial", 16),
                          command=create_action_handler(top_icon)).pack(side="left", padx=2)

        # 4. Scrollable Frame untuk Tabel (FIX ERROR)
    ...
    def render_table(self, data):
        ...
        for row_data in data:
            ...
            ctk.CTkLabel(name_frame, text=row_data[0], font=("Arial Bold", 13), 
                         anchor="center", padx=50).pack(side="left")  # ← Tambahkan padx=15

            ctk.CTkLabel(row_frame, text=row_data[1], width=col_widths[1],
                         anchor="center", font=("Arial", 13), padx=100).pack(side="left", padx=10)

            ctk.CTkLabel(row_frame, text=row_data[2], width=col_widths[2],
                         anchor="center", font=("Arial", 13), padx=30, pady=20).pack(side="left", padx=70)
            ...
            # Tombol Detail (Tanpa Lambda)
            ctk.CTkButton(row_frame, text="👁",
                          text_color="#01030E",
                          width=40, height=40,
                          fg_color="transparent",
                          hover_color="#3B8ED0",
                          # Memanggil fungsi create_detail_handler untuk mengikat data ke tombol
    ...
    def search_patient(self, keyword):
        ...
        result = []
        # Menggunakan loop for biasa (bukan list comprehension) dan pengecekan non-case-sensitive
